---
title: "Simple Demo of Repeated Measures ANOVA/Mixed Model Equivalence"
output:
  pdf_document:
    toc: yes
  html_document:
...
Some are still trained statistically with a heavy focus on analysis of variance (ANOVA) techniques, but  without delving too much into their role as special cases of regression, and mixed models more generally. This is despite the fact that data collected is typically richer than a couple of tightly controlled treatment conditions with a normally distributed target variable, rendering these techniques too restrictive for many common situations. The following uses a very small dataset to demonstrate the connections to standard linear and mixed models, with a hope that some will consider using the more general approaches after realizing they are only gaining in flexibility while maintaining the same analyses of interest they are already using.
...
When I meant the effect is eyeballable, everyone in the treatment group improves from pre to post, while controls are mixed at best, and this effect is visually obvious.
...
Since [Lord's Paradox]() was introduced, there has been some back and forth among various papers ever since debating whether one should use ANCOVA vs. t-test on change scores.  Aside from it not really being a paradox (both statisticians are correct), it's unclear to me that the debate is very useful for applied researchers for various reasons.  For one, I've never seen a study in which I'd be interested in the results of a 2x2 design.  It's overly simplistic and doesn't generalize to different types of outcomes that are inherently non-normal (e.g. binary variables), which are standard fare in the scientific world.  As such, a t-test on gain scores is never going to be an option outside of a perhaps misguided use as a descriptive statistic. Some are concerned about statistical power, but basing your assessment of treatment viability based solely on an arbitrary p-value cutoff is borderline unethical for some lines of research (in my opinion), so that shouldn't be the overriding concern either. In addition, whether observational or experimental, a statistical result will not tell you about causal relations, so concerns about experimental vs. observational design are perhaps off-base in many data scenarios.  You can, however, specify those causal effects that you think exist and run a model appropriate to the situation, focusing on the specific effect of interest.  In this case the t-test and ANCOVA can be seen as different effects, *total* and *direct* respectively, from the same causal model (see [Pearl, 2014](http://ftp.cs.ucla.edu/pub/stat_ser/r436.pdf))
...
In addition, there is actually a way to convert the t-test on change scores to the ANCOVA result, such that, *if there is a strong correlation between pre and post*, one can find an equivalence (using the squared t vs. the ANCOVA F) with increasing sample size.  The code in the [Appendix](#t-test-ancova-equivalence) demonstrates this.

### Repeated Measures Anova

Now that we've gone through that, let's do a repeated measures ANOVA.
...
Note that the interaction F-value here is the squared value of the t-test on the change scores (`r round(ttestChange0$statistic^2, 2)`).  In other words, a t-test on the change score is the interaction term from the repeated measures ANOVA on the same data.
