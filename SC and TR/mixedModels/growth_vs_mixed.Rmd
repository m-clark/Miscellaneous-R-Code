...
knitr::opts_chunk$set(message=F, fig.align='center', cache=T)
```
...
## SEM
In the following we'll set up the SEM in lavaan with appropriate constraints, plus one additional constraint to make direct comparisons to the mixed model with nlme.  We will hold off on results for now, but go ahead and display the graph pertaining to the conceptual model.  At this point we're simply estimating the variances of the latent variable and the residual variances of the observed data.

In this setting the means for the *y* are zero and will be fixed as such.

```{r lavaanmod, fig.align='center'}
library(lavaan)
LVmodel1 = '
I =~ 1*y1 + 1*y2 + 1*y3
y1 + y2 + y3 ~ 0*1             # fix y intercepts to zero
'  

semres1 = sem(LVmodel1, y)
```
...
Now we will investigate random intercepts and slopes as in a standard 'growth curve model'.  For this data the *y* is a repeated measurement over time, otherwise the data is much the same as before. However, we will add a slight positive correlation between intercepts and slopes, and scale time to start at zero so the intercept represents the average baseline value.

## Data
The main difference here is adding a covariate for time and a second latent variable. For this demo, the 'fixed' effects in the mixed model sense will be zero.  The variances of the latent variables are set to one.  In addition the sample size is notably increased, which is mostly done to help the nlme estimates get closer to the mark.  I would also mention that lavaan takes a split second to estimate the model while nlme takes probably half a minute on my computer. This would be interesting to investigate further, as a large sample size is necessary for any SEM to be worthwhile, but perhaps after
...
## Models
For the SEM we now have two latent structures, one representing the random intercepts, another the random slopes for time.  For the mixed model we specify both random intercepts and slopes, and bump up the iterations, otherwise it will stop before converging on its own.  The graphical model for the SEM is shown.
...
- Time-varying covariates:  With time-varying covariates, I don't think people are usually interested in the multivariate approach where each time point of the dependent variable must be predicted by each time point of the covariate, and the model gets unwieldy very quickly with only handful of time-varying covariates. With many time points, the coding and output is also messy with the SEM approach.
- Nonlinear relationships: While one can incorporate nonlinear relationships very easily in the standard setting due to the close ties between mixed and additive models, it becomes cumbersome (adding a quadratic slope factor) or unusual to interpret (allowing the slope loadings to be estimated, rather than fixed) in the SEM setting.
...
- Parallel Processes: Within the Bayesian framework one can incorporate [parallel processes] (multivariate mixed models) via a multivariate outcome fairly easily with the [brms package](https://github.com/paul-buerkner/brms) ([example](https://github.com/mclark--/MultivariateMultiLevelModeling/blob/master/multivariate.knit.md) with Stan).
- Indirect Effects: Indirect effects can also be incorporated in the standard mixed model framework ([example with Stan](https://github.com/mclark--/Miscellaneous-R-Code/blob/master/ModelFitting/Bayesian/rstan_multilevelMediation.R), see also the [mediation](https://cran.r-project.org/package=mediation) package for using lme4 for multilevel mediation). 
- Sample sizes: SEM is an inherently large sample technique, and growth curve models can quite complicated in terms of the number of parameters to be estimated. Obviously large samples are always desirable for either approach, but e.g. where mixed models can be run on clustered data with 30 clusters, it would be a bit odd to use SEM for 30 observations.  
...
In short, my opinion is that growth curve models would only be preferred in settings where the model is notably complicated, but the level of complexity would start at the point where the interpretation of such a model would already be very difficult. SEM has its place, but the standard mixed model approach should be preferred, even in fairly complicated settings, both for ease of implementation and ease of interpretation.
...
[^varIdent]: lme actually works by setting the reference group variance to 1, and the coefficients represent the ratios of the other variances to that group. So in this case, it doesn't actually estimate the y1 variance, but as it 
See `?varIdent`.