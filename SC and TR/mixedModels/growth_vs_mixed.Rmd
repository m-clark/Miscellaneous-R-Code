...
- <span class="emph">Sample sizes</span>: SEM is an inherently large sample technique, and growth curve models can become quite complicated in terms of the number of parameters to be estimated. Obviously large samples are always desirable for either approach, but e.g. where mixed models can be run on clustered data with 30 clusters, it would be a bit odd to use SEM for 30 observations. I have some simulation results [here]().  One would need at least 50 clusters with many data points within each for the growth curve model to approach the performance of the mixed model, in which setting even a few clusters with few time points is okay.
