...
### Intro
The following is a function for estimating the parameters of a linear regression via variational inference. See my other scripts for Stan implementation via Hamiltonian Monte Carlo.  See Drugowitsch (2014) for an overview of the method outlined in Bishop (2006).

I will use the notation in Drugowitsch in most cases. Here w, represents the coefficients, and $\tau$ the precision (inverse variance). The likelihood for response y is $\mathcal{N}(Xw, \tau^{-1})$. Priors for w and tau are normal inverse gamma $\mathcal{N}(0, (\tau\alpha)^{-1}) Gamma(a0,b0)$. See [here](https://github.com/jdrugo/vb_linear/blob/master/vb_linear_fit.m) and [here](https://github.com/jdrugo/vb_linear/blob/master/vb_linear_fit_ard.m) for his matlab implementation.
...
mseResults = data.frame(vb=rbind(vbTrainError, vbTestError), glm=rbind(glmTrainError, glmTestError))
rownames(mseResults) = c('train', 'test')
mseResults
```

In general the results are as expected where the standard approach overfits relative to vb reg.  We can visualize the results as well.

```{r visResults, cache=TRUE}
library(ggvis)
gcoef = data.frame(wGLM=coef(glmResult), wVB=vbResult$coef, w=coefs)
...
And now a notably higher dimension case with irrelevant predictors as in Drugowitsch section 2.6.3.  This will take a while.
...
mseResults = data.frame(vb=rbind(vbTrainError, vbTestError), vbARD=rbind(vbARDTrainError, vbARDTestError), 
...
```{r visResults2, cache=TRUE}
library(ggvis)
gcoef = data.frame(wGLM=coef(glmResult), wVB=vbResult$coef, wVBARd = vbARDResult$coef, w=c(coefs, rep(0,d-deff)))
gcoef = reshape2::melt(gcoef, 'w')
...
gpred %>%
  ...
  layer_lines(~y, ~y, strokeOpacity:=.5) %>%
  layer_points(fill=~variable, fillOpacity:=.75) %>%
  scale_numeric("y", domain = c(-15, 15))
```
