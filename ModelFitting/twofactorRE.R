...
### piglets in each litter.  See onefactor.R for a one factor model, and        ###
### twofactorRE.m and twofactorRE.jl for the matlab and julia versions of this  ###
### example. Note that the text has a typo on the sigma2 variance estimate      ###
...
sfran_loglike = function(mu, eta_alpha, eta_gamma, eta){
  # Args are mu: intercept; eta_alpha: random effect of sire; eta_gamma: random
  # effect associated with sire and dam; eta: residual variance of y
  ...
  Sigma = sigma2*diag(n) + sigma2_alpha*tcrossprod(Xalpha) + sigma2_gamma*tcrossprod(Xgamma);
...
################################
starts = list(mu=mean(y), eta_alpha=var(tapply(y, d$sire, mean)), 
              eta_gamma=var(y)/3, eta=var(y)/3)
