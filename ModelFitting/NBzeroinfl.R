...
ZINB = function(y, X, par) {
  ...
  # LLs
  logliklogit = log( logi0 + exp(log(1-logi0) + suppressWarnings(dnbinom(0, size=theta, mu=munb, log=T))) )
  logliknegbin = log(1-logi0) + suppressWarnings(dnbinom(y, size=theta, mu=munb, log=T))
...
starts = c(negbin=coef(startcount), logit=coef(startlogi), theta=1)  
# starts = c(negbin=rep(0, 3), logit=rep(0, 3), theta=log(1))  


### Estimate with optim function ###
optNB1 = optim(par=starts , fn=ZINB, X=init.mod, y=fish$count, 
               method="BFGS", control=list(maxit=5000, reltol=1e-12), hessian=T)
# optNB1
...
starts = c(negbin=coef(startcount), logit=coef(startlogi), theta=summary(startcount)$dispersion)  
# starts = c(negbin=rep(0, 6), logit=rep(0, 6), theta=1)


### Estimate with optim function ###
optNB2 = optim(par=starts , fn=ZINB, X=init.mod, y=bioChemists$art, 
               method="BFGS", control=list(maxit=5000, reltol=1e-12), hessian=T)
# optNB2
