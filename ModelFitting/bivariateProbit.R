...
probitLL = function(beta, X, y){
  mu = X %*% beta
  # ll = sum(dbinom(y, size=1, prob=pnorm(mu), log=T))               # these produce identical results, but the second is the typical depiction
...
# Example at http://www.ats.ucla.edu/stat/stata/dae/probit.htm

admit = haven::read_dta('http://www.ats.ucla.edu/stat/stata/dae/binary.dta')
head(admit)

X = model.matrix(admit~ gre + gpa + factor(rank), admit)
y = admit$admit
init = rep(0, ncol(X))
optimResult = optim(fn=probitLL, par=init, X=X, y=y, method='BFGS')
optimResult 
...
y = auto$foreign
init = rep(0, ncol(X))
optimResult = optim(fn=probitLL, par=init, X=X, y=y)
optimResult
...
optimResult = optim(fn=bivariateProbitLL, par=init, X=X, y1=y1, y2=y2, method='BFGS')
