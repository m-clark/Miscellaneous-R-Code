...
```{r}
rk.1 = function(s, t) {
  return(.5 * min(s, t)^2 * max(s, t) - (1/6) * min(s, t)^3)  # bug in original code, now corresponds to article
}
```
...
This is from the supplemental section for the article at https://www.tandfonline.com/doi/suppl/10.1080/00031305.2012.678196?scroll=top.  I used Rstudio's default cleanup to make the code a little easier to read, and maybe added a little spacing, but otherwise it is identical to what's in the linked paper.
